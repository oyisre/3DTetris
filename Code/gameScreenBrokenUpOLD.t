    ...
    if (Time.Elapsed () - curTime) > (1000 / 60) then
        ...
        if isChanged = true then
            ...
            for i : 0 .. maxX+1
                ...
                GL.DrawLine (sX + (i * sideL), sY, sZ, sX + (i * sideL), sY, sZ - (maxX+1 * sideL), 255, 255, 255)
            end for
                
            %Main Fields (Front, Left, Back, Right)
            if curRotationY >= -90 and curRotationY <= 90 then
                for i : 0 .. 16
                    GL.DrawLine (sX, sY + (i * sideL), sZ, sX + (12 * sideL), sY + (i * sideL), sZ, 255, 255, 255)
                end for
                    for i : 0 .. 12
                    GL.DrawLine (sX + (i * sideL), sY, sZ, sX + (i * sideL), sY + (16 * sideL), sZ, 255, 255, 255)
                end for
                    GL.DrawLine (sX, sY, sZ, sX + (12 * sideL), sY, sZ, 255, 0, 0)
                reverseKeys := false
            end if
            if curRotationY >= 0 and curRotationY <= 180 then
                for i : 0 .. 16
                    GL.DrawLine (sX, sY + (i * sideL), sZ, sX, sY + (i * sideL), sZ - (12 * sideL), 255, 255, 255)
                end for
                    for i : 0 .. 12
                    GL.DrawLine (sX, sY, sZ - (i * sideL), sX, sY + (16 * sideL), sZ - (i * sideL), 255, 255, 255)
                end for
                    GL.DrawLine (sX, sY, sZ, sX, sY, sZ - (12 * sideL), 0, 255, 0)
            end if
            if curRotationY >= 90 or (curRotationY >= -180 and curRotationY <= -90) then
                for i : 0 .. 16
                    GL.DrawLine (sX, sY + (i * sideL), sZ - (12 * sideL), sX + (12 * sideL), sY + (i * sideL), sZ - (12 * sideL), 255, 255, 255)
                end for
                    for i : 0 .. 12
                    GL.DrawLine (sX + (i * sideL), sY, sZ - (12 * sideL), sX + (i * sideL), sY + (16 * sideL), sZ - (12 * sideL), 255, 255, 255)
                end for
                    GL.DrawLine (sX, sY, sZ - (12 * sideL), sX + (12 * sideL), sY, sZ - (12 * sideL), 255, 0, 0)
                reverseKeys := true
            end if
            if curRotationY >= -180 and curRotationY <= 0 then
                for i : 0 .. 16
                    GL.DrawLine (sX + (12 * sideL), sY + (i * sideL), sZ, sX + (12 * sideL), sY + (i * sideL), sZ - (12 * sideL), 255, 255, 255)
                end for
                    for i : 0 .. 12
                    GL.DrawLine (sX + (12 * sideL), sY, sZ - (i * sideL), sX + (12 * sideL), sY + (16 * sideL), sZ - (i * sideL), 255, 255, 255)
                end for
                    GL.DrawLine (sX + (12 * sideL), sY, sZ, sX + (12 * sideL), sY, sZ - (12 * sideL), 0, 255, 0)
            end if
            
            %Reference Lines
            if curRotationY >= 0 and curRotationY < 90 then
                GL.DrawLine (sX, sY, sZ, sX, sY + (16 * sideL), sZ, 0, 0, 255)
            elsif curRotationY >= 90 and curRotationY < 180 then
                GL.DrawLine (sX, sY, sZ - (12 * sideL), sX, sY + (16 * sideL), sZ - (12 * sideL), 0, 0, 255)
            elsif curRotationY >= -180 and curRotationY < -90 then
                GL.DrawLine (sX + (12 * sideL), sY, sZ - (12 * sideL), sX + (12 * sideL), sY + (16 * sideL), sZ - (12 * sideL), 0, 0, 255)
            elsif curRotationY >= -90 and curRotationY < 0 then
                GL.DrawLine (sX + (12 * sideL), sY, sZ, sX + (12 * sideL), sY + (16 * sideL), sZ, 0, 0, 255)
            end if
        ...
        if key ('r') xor key ('f') then
            if key ('r') then
                ...
                if scaleFactor > -50 then             % set to -3
                    scaleFactor -= 1
                    rotate ()
                end if
            end if
        ...
        /*
        if key (' ') then
        if inPlay = true and lastKeyPress not= 7 and Time.Elapsed - lastKeyDelay > 75 then
        for i : 0 .. 18
        moveDown ()
        end for
            lastKeyDelay := Time.Elapsed
        lastKeyPress := 7
    elsif inPlay = false and Time.Elapsed - lastKeyDelay > 250 then
        solidify ()
        canWin := canSpawn ()
        if canWin = true then
        spawnPiece ()
